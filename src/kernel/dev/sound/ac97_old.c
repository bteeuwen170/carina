...
static void int_handler(struct int_stack *regs)
{
	...
	u16 samples = io_inw(bar_nabm + 0x18);

	if (!curbuf || samples < 1)
		io_outw(bar_nabm + 0x15, 32);

	io_outw(bar_nabm + 0x16, (1 << 3));
...
{
	io_outw(bar_nam + 0x02, (volume << 8) | volume);
	io_outw(bar_nam + 0x04, (volume << 8) | volume);
	io_outw(bar_nam + 0x06, volume);
	io_outw(bar_nam + 0x0A, volume);
	io_outw(bar_nam + 0x18, (volume << 8) | volume);
}
...
void ac97_play(void)
{
	...
	io_outb(bar_nabm + 0x15, 32);
	io_outb(bar_nabm + 0x1B, (1 << 0) | (1 << 3) | (1 << 4));
}
...
void ac97_init(void)
{
	...
	/* TODO */
	bar_nam = pci_ind(card->bus, card->dev, card->func, 0x10);
	bar_nabm = pci_ind(card->bus, card->dev, card->func, 0x10 + 0x4 * 1);

	/* Power the card on */
	io_outd(bar_nabm + 0x2C, io_ini(bar_nabm + 0x2C) | (1 << 0x01));
	sleep(50);

	/* Reset the card */
	io_outd(bar_nam, 0x01);
	sleep(50);
	io_outd(bar_nabm + 0x2C, io_ini(bar_nabm + 0x2C) | (1 << 0x02));
	...
	io_outd(bar_nabm + 0x10, (intptr_t) buf);
}
